...
BEGIN TRY
	...
	BEGIN
			...
			DECLARE @EventTime SMALLDATETIME
			...
			ELSE IF @PhysicianText IS  NULL AND NOT EXISTS 
			(
			...
			ELSE IF @SigningPhysicianID IS  NULL AND  @PhysicianText IS NOT NULL AND @PhysicianText <>''
			BEGIN
				IF @EventTIme IS NULL
				BEGIN
				EXEC p_EHR_Incomplete @moduleKey, 't_EHR_OperativeReport', 'Signature will be enabled after the Procedure/ Incision time is set';
				END
				ELSE
				BEGIN
